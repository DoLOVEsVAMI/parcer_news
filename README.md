...
- `--embed-images` — встраивать фото в HTML как data URI (может работать заметно дольше на больших объемах)
...
- По умолчанию фото вставляются в HTML как обычные `<img src="https://...">` (быстро).
- Если нужен полностью самодостаточный HTML без внешней загрузки картинок, используйте `--embed-images`.
- Во время генерации HTML в консоли показывается отдельный progress bar.
...
- В API-режиме выводится progress bar загрузки постов с VK.
- После загрузки показывается progress bar обработки/классификации постов по каждому источнику.

## Важно про `--count` в API режиме

- VK API возвращает максимум 100 постов за один запрос `wall.get`.
- Скрипт автоматически делает пагинацию, поэтому `--count 1000` теперь действительно пытается собрать до 1000 постов (если они есть).
